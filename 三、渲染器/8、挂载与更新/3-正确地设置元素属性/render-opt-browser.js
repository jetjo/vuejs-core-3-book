...
import baseConfig from '../1-挂载子节点与元素属性/render-opt-jsdom.js'
 ...
 * @param {string} key
 * */
export function isReadonlyIDLAttr(ele, key) {
 ...
 * @param {string} key
 * */
export function shouldSetAsProp(ele, key) {
  if (isReadonlyIDLAttr(ele, key)) return false
  return key in ele
}

/**@type {typeof baseConfig.patchProps} */
const patchProps = function (el, key, _, nextValue) {
...
patchProps.isElement = function (n) {
